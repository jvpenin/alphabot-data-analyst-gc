...
function appendMessage(text, sender = 'bot') {
  ...
  // Configura a exibição do texto
  bubble.style.whiteSpace = 'pre-wrap';
  bubble.style.wordWrap = 'break-word';
  bubble.style.lineHeight = '1.6';
  
  // Para respostas do bot, processa o texto para melhor formatação
  if (sender === 'bot') {
    // Substitui múltiplas quebras de linha por espaçamento adequado
    const formattedText = text
      .replace(/\n{3,}/g, '\n\n') // Máximo 2 quebras consecutivas
      .replace(/^\s+|\s+$/g, '') // Remove espaços no início/fim
      .replace(/([.!?])\s*([A-ZÁÊÇÔ])/g, '$1\n\n$2'); // Quebra após pontos antes de maiúsculas (novo parágrafo)
...
// Mensagem inicial
appendMessage('Olá! Eu sou o Alpha Bot, um assistente de análise de dados. Envie uma planilha e me faça perguntas sobre ela!', 'bot');
